...
public class HeapPage implements Page {
   ...
   * 
   * @param td TupleDesc of tuples on this page
   * @return the number of tuples on this page
   */
  private static int getNumTuples(TupleDesc td) {
   ...
   * 
   * @param numberOfTupleSlots number of tuple slots on this page
  ...
  private Tuple readNextTuple(DataInputStream dis, int slotId) throws NoSuchElementException {
    ...
    try {
      for (int j = 0; j < tupleDesc.numFields(); j++) {
        Field f = tupleDesc.getType(j).parse(dis);
        t.setField(j, f);
      }
  ...
  protected int getNextTupleIndex(int startIndex, int maxIndex) {
    ...
    while (nextTupleIndex < maxIndex && !getSlot(nextTupleIndex)) {
      nextTupleIndex++;
    }
