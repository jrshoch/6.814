...
public class OperatorCardinality {
    ...
    if (o instanceof Filter) {
      ...
      if (children.length > 0 && children[0] != null) {
        if (children[0] instanceof Operator) {
          ...
          childC = tableStats.get(((SeqScan) children[0]).getTableName()).estimateTableCardinality(
              1.0);
        }
      }
  ...
  private static boolean updateFilterCardinality(Filter f, Map<String, Integer> tableAliasToId,
    ...
    if (tableId != null) {
      selectivity = tableStats.get(Database.getCatalog().getTableName(tableId.intValue()))
          .estimateSelectivity(
              Database.getCatalog().getTupleDesc(tableId.intValue())
                  .fieldNameToIndex(pureFieldName), pred.getOp(), pred.getOperand());
  ...
  private static boolean updateJoinCardinality(Join j, Map<String, Integer> tableAliasToId,
    ...
    boolean child1HasJoinPK = Database.getCatalog()
        .getPrimaryKey(tableAliasToId.get(tableAlias1).intValue()).equals(pureFieldName1);
    boolean child2HasJoinPK = Database.getCatalog()
        .getPrimaryKey(tableAliasToId.get(tableAlias2).intValue()).equals(pureFieldName2);
    ...
    j.setEstimatedCardinality(JoinOptimizer.estimateTableJoinCardinality(j.getJoinPredicate()
        .getOperator(), tableAlias1, tableAlias2, pureFieldName1, pureFieldName2, child1Card,
        child2Card, child1HasJoinPK, child2HasJoinPK, tableStats, tableAliasToId));
  ...
  private static boolean updateAggregateCardinality(Aggregate a,
    ...
    if (tableId != null) {
      groupFieldAvgSelectivity = tableStats.get(
          Database.getCatalog().getTableName(tableId.intValue())).avgSelectivity(
          Database.getCatalog().getTupleDesc(tableId.intValue()).fieldNameToIndex(pureFieldName),
          Predicate.Op.EQUALS);
