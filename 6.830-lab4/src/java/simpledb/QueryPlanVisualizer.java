...
public class QueryPlanVisualizer {
  ...
  private SubTreeDescriptor buildTree(int queryPlanDepth, int currentDepth, DbIterator queryPlan,
    ...
    if (queryPlan instanceof SeqScan) {
      ...
      if (plan instanceof Join) {
        ...
        thisNode.text = String.format("%1$s(%2$s),card:%3$d", JOIN, field1 + jp.getOperator()
            + field2, Integer.valueOf(j.getEstimatedCardinality()));
        int upBarShift = parentUpperBarStartShift;
        if (JOIN.length() / 2 > parentUpperBarStartShift)
          upBarShift = JOIN.length() / 2;

        SubTreeDescriptor left = this.buildTree(queryPlanDepth, currentDepth + adjustDepth + 3,
            children[0], currentStartPosition, upBarShift);
        SubTreeDescriptor right = this.buildTree(queryPlanDepth, currentDepth + adjustDepth + 3,
            children[1], currentStartPosition + left.width + SPACE.length(), 0);
        ...
        String field1 = td.getFieldName(jp.getField1());
        String field2 = td.getFieldName(jp.getField2() + children[0].getTupleDesc().numFields());
        thisNode.text = String.format("%1$s(%2$s),card:%3$d", HASH_JOIN, field1 + jp.getOperator()
            + field2, Integer.valueOf(j.getEstimatedCardinality()));
        int upBarShift = parentUpperBarStartShift;
        if (HASH_JOIN.length() / 2 > parentUpperBarStartShift)
          upBarShift = HASH_JOIN.length() / 2;
        SubTreeDescriptor left = this.buildTree(queryPlanDepth, currentDepth + 3 + adjustDepth,
            children[0], currentStartPosition, upBarShift);
        SubTreeDescriptor right = this.buildTree(queryPlanDepth, currentDepth + 3 + adjustDepth,
            children[1], currentStartPosition + left.width + SPACE.length(), 0);
        thisNode.upBarPosition = (left.upBarPosition + right.upBarPosition) / 2;
        ...
        if (gfield == Aggregator.NO_GROUPING) {
          thisNode.text = String.format("%1$s(%2$s),card:%3$d", a.aggregateOp(),
              a.aggregateFieldName(), Integer.valueOf(a.getEstimatedCardinality()));
          alignTxt = td.getFieldName(00);
        } else {
          thisNode.text = String.format("%1$s(%2$s), %3$s(%4$s),card:%5$d", GROUPBY,
              a.groupFieldName(), a.aggregateOp(), a.aggregateFieldName(),
              Integer.valueOf(a.getEstimatedCardinality()));
          alignTxt = GROUPBY;
        }
        if (alignTxt.length() / 2 > parentUpperBarStartShift)
          upBarShift = alignTxt.length() / 2;

        SubTreeDescriptor child = this.buildTree(queryPlanDepth, currentDepth + 2 + adjustDepth,
            children[0], currentStartPosition, upBarShift);
        ...
        thisNode.text = String.format("%1$s(%2$s),card:%3$d", SELECT, children[0].getTupleDesc()
            .getFieldName(p.getField()) + p.getOp() + p.getOperand(),
            Integer.valueOf(f.getEstimatedCardinality()));
        int upBarShift = parentUpperBarStartShift;
        ...
        thisNode.text = String.format("%1$s(%2$s),card:%3$d", ORDERBY, children[0].getTupleDesc()
            .getFieldName(o.getOrderByField()), Integer.valueOf(o.getEstimatedCardinality()));
        int upBarShift = parentUpperBarStartShift;
        ...
        thisNode.text = String.format("%1$s(%2$s),card:%3$d", PROJECT, fields,
            Integer.valueOf(p.getEstimatedCardinality()));
        int upBarShift = parentUpperBarStartShift;
        ...
        Integer card = Integer.valueOf(0);
        try {
        ...
        thisNode.text = String.format("%1$s,%2$s->%3$s,card:%4$d", RENAME, oldName, newName,
            Integer.valueOf(plan.getEstimatedCardinality()));
        int upBarShift = parentUpperBarStartShift;
  ...
  private void printTree(SubTreeDescriptor root, char[] buffer, int width) {
    ...
    if (root.leftChild != null && root.rightChild == null) {
      ...
      Arrays.fill(buffer, (textHeight + 1) * width + root.leftChild.upBarPosition, (textHeight + 1)
          * width + root.rightChild.upBarPosition + 1, '_');
  ...
  public void printQueryPlanTree(DbIterator physicalPlan, PrintStream out) {
    PrintStream notNullOut = (out == null) ? System.out : out;

    String tree = this.getQueryPlanTree(physicalPlan);

    notNullOut.println(tree);
    notNullOut.close();
  }
}
