...
public class QueryTest {
   ...
   * nested-loops join is available. The tables are also too big for a query to
   * be successful if its query plan isn't reasonably efficient, and there are
   * too many tables for a brute-force search of all possible query plans.
   */
   ...
   * 
   * // Each of these should return around 20,000 // This Parser implementation
   * currently just dumps to stdout, so checking that isn't terribly clean. //
   * So, don't bother for now; future TODO. // Regardless, each of the following
   * should be optimized to run quickly, // even though the worst case takes a
   * very long time. Parser.processNextStatement(
